...
1. **自由與群組分開**:自由 product 不與群組 product 同住。每台實體機屬於**單一共居類別** —— 自由開放池(只住自由 product,彼此可混)/ 某一群組(只住同群)/ 單一獨佔 VM。三者互不混用。
2. **vcore 非 VM 尺寸整數倍時**:**無條件進位成整台 + 警告**(與現行「每月無條件進位」一致,不報錯)。
3. **v1 不做獨佔近似**:獨佔**只收資料、不在 v1 packing 強制**,等 CP-SAT。(簡化 v1)
4. **群組相容性只有「同名可共用」**:無群組間相容關係(A 可與 B)。保持最簡,不夠用再議。
